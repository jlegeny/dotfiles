...
# Updated by Jozef Legeny 2013-01-01
# http://clockwork.fr
# Added guards to ignore ^format commands so the script plays nicely. Added a
# last; statement in the server loop as irssi sends the away command to all
# servers anyways.
...
sub auto_timeout {
	...
	foreach my $server (Irssi::servers()) {
		$server->command("/AWAY AFK");
		#Irssi::print("Going AWAY");
		Irssi::command('^format timestamp {timestamp %G%%H%%M%w }');
		last;
	}
...
sub reset_timer {
	...
	if ($autoaway_state eq 1) {
		...
		foreach my $server (Irssi::servers()) {
			$server->command("/AWAY");
			#Irssi::print("Going BACK");
			Irssi::command('^format timestamp {timestamp %K%%H%%M%w }');
			last;
		}
